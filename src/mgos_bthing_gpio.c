...
bool mg_bthing_gpio_get_state_cb(mgos_bthing_t thing, mgos_bvar_t state, void *userdata) {
  ...
  if (thing && state && cfg) {
    ...
    mgos_bvar_set_bool(state, (cfg->pull == MGOS_GPIO_PULL_UP ? !gpio_is_on : gpio_is_on));
    return true;
  }
  return false;
}
...
bool mg_bthing_gpio_set_state_cb(mgos_bthing_t thing, mgos_bvarc_t state, void *userdata) {
  ...
  if (thing && cfg && (mgos_bvar_get_type(state) == MGOS_BVAR_TYPE_BOOL)) {
    ...
    mgos_gpio_write(cfg->pin, (cfg->pull == MGOS_GPIO_PULL_UP ? !gpio_set_on : gpio_set_on));
    return true;
  }
  return false;
}
...
bool mgos_bthing_gpio_attach(mgos_bthing_t thing, int pin, enum mgos_gpio_pull_type pull) {
  ...
  if (mgos_bthing_is_typeof(thing, MGOS_BTHING_TYPE_ACTUATOR)) {
    ...
    if (!mgos_gpio_setup_output(pin, (pull == MGOS_GPIO_PULL_UP ? true : false))) return false;
